<?php 
class Offerstranslation_model extends CI_Model {	
	...
	function inserthome($data){	
		...
		$renterdata["lang['language']"] = $data["lang['language']"];
		$renterdata["lang['renter_lang']"] = $data["lang['renter_lang']"];unset($data["lang['renter_lang']"]);
		...
		if(isset($updated_id)){
			...
			// renter
			$renter_lang=$location.$folderName.'/'.'renter_lang'.$extension;
			$renter_file = fopen($renter_lang, "wb") or die("Unable to open file!");
			$renter_txt = '<?php ';
			foreach($renterdata as $key=>$value){
				$renter_txt .='$'.$key.' = "'.$value.'";'."\r\n";
			}
			$renter_txt .=' ?>';
			fwrite($renter_file, $renter_txt);
			fclose($renter_file);
	...
	public function get_languages(){
		$this->db->select('general_languages.*,admin_role.rolename');
		$this->db->from('general_languages' );
		$this->db->join('admin_role', 'admin_role.id = general_languages.created_by','left');
